...
namespace format {  // to format output

auto find_braces(std::string_view s_p) {
...
struct format {
    explicit format(std::string_view);
    template <typename T>
    explicit format(std::string_view, T t);
    template <typename T, typename... Ts>
    explicit format(std::string_view, T t, Ts... ts);
};
format::format(std::string_view s) { std::cout << s << "\n"; }
template <typename T>
void func(std::string_view s, T t) {
    int i = find_braces(s);
    if (i == -1) {
...
int c = 0;
template <typename T, typename... Ts>
void func(std::string_view s, T t, Ts... ts) {
...
template <typename T>
format::format(std::string_view s, T t) {
    func(s, t);
}
template <typename T, typename... Ts>
format::format(std::string_view s, T t, Ts... ts) {
    func(s, t, ts...);
}
}  // namespace format
...
auto make_binary(auto x) -> int { return static_cast<int>(x); }

struct Hash_Container {
    Hash_Container(auto i) { auto x = make_binary(i); }
};
}  // namespace containers

namespace stack_manipulation {
void merge_two_vectors_into_v_of_pair_of_string_int(
    std::vector<std::string_view> s, std::vector<int> i_,
    std::vector<pair_<std::string_view, int>>& vpsi) {
    // vpsi
    for (int i = 0; i < s.size() and i < i_.size(); i++) {
        std::string_view x = s[i];
        int y = i_[i];
        pair_<std::string_view, int> p{x, y};
        vpsi.push_back(p);
    }
}

template <typename T>
[[nodiscard]] auto get_word_count_in_nodes(node<T>& start_node,
                                           bool is_part_of_stack)
    -> std::vector<pair_<std::string_view, int>> {
    node<T>& current_node = start_node;

    std::vector<pair_<std::string_view, int>> v{};  // vector to return

    std::vector<std::string_view> v_s{};  // vector for storing strings or words
    std::vector<int> v_i{};     // vector for storing ints or counts of words
 assert(is_part_of_stack);  // just make sure because it is in
                                // stack_manipulation namespace.

    while (current_node.is_member_of_stack) {
        v_s.push_back(
            current_node
                .data_associated_with_node);  // push back data with node
        auto x = std::find(
            v_s.begin(), v_s.end(),
            current_node.data_associated_with_node);  // find index of where
                                                      // just pushbacked
        v_i[x] = v_i[x] + 1;
...
template <typename T>
auto go_through_stack_encrypt(stacktrace<T>& stack_p) {
...
// template<typename T> // dont need this can use the generic one instead.
// warning from pvs-studio
// [[nodiscard]] auto go_through_stack_decrypt(stacktrace<T>& stack_p)
// {
//     auto x = stack_p.current_node;
//     for(int i = 0; i < stack_p.size(); i++)
//     {
...
template <typename T>
[[nodiscard]] auto go_through_stack(
    T& stack_p, bool b)  // true if incrypt or false decrypt.
{
    auto x = stack_p.current_node;
    for (int i = 0; i < stack_p.size(); i++) {
        if (!b) {
            x = decryption(stack_p.current_node);
        } else {
            x = incryption(stack_p.current_node);
        }
        stack_p.current_node = stack_p.current_node.next;
    }
    stack_p.current_node = x;
...
struct node {
    ...
    node(node<T>& x)  // copy constructor, pvs-studio gave error
    {
        data_associated_with_node = x.data_associated_with_node;
        is_member_of_stack = x.is_member_of_stack;
    }
...
struct stack {
    ...
    stack(node<T> node_jst_bfore = node<T>{}, node<T> node_curr = node<T>{},
          node<T> lst_node = node<T>{}, node<T> tmp_node = node<T>{}) {
        node_just_before = node_jst_bfore;
        current_node = node_curr;
        last_node = lst_node;
        temp_node = tmp_node;
    }
    ...
    auto size() { return find_amount_of_nodes(); }
};
...
void stack<T>::initialize_nodes(X x, Xs... xs) {
    if (is_first_initialize_nodes) {
        constant_node.data.data_associated_with_node =
            x /*.data_associated_with_node*/;  ///////////////////////// changed
                                               ///from A
    }
    current_node.data_associated_with_node =
        x;  /////////////////////////////////////////// changed from A
...
template <typename T>
struct stacktrace : stack<T> {
    auto encrypt_data() -> bool;  // initialized elsewhere
                                  // bool signifies if encryption went well
};

template <typename T>
auto stacktrace<T>::encrypt_data() -> bool {
    // so have a Hash_Container map. Fill it with common keys:
    // Fill it with common keys by going throught each node incrementing a count
    // for word and which ever node is deepest use the most encryption and
    // shallow least encryption. Do this by encryption_depth_score
...
auto add_action_to_stacktrace(stacktrace<T>& program_s, Action<B> act) {
    program_s.current_node = act;
    program_s.current_node = program_s.current_node.next;
}

// a structural string. Couldnt use std::string_view in stacktrace type
struct str {
    char* string_data;
...
int main() {
    ...
    // p_s.initialize_nodes<make_node(5), make_node(3), make_node(6),
    // make_node(2), make_node(7), make_node(4), make_node(1)>();

    p_s.current_node = make_node(5);

    // format teting.
    format::format f("Hello my name is {}", 'R');
    onl();
    format::format xyz("Good Afternoon {}", "C++");
    onl();
    format::format abc("Good Afternoon {}", 42);
    onl();
    containers::binary<1, 0, 1, 0, 0, 1>
        b;  // for some reason gives a really wierd output.
    onl();
    b.output_bits();
    ...
    stack<int> xyzY{};
    // xyzY.initialize_nodes(1, 2, 3, 4); // this is what is causing the wrong
    // output.
    std::cout << "xyz before incryption: "
              << xyzY.current_node.data_associated_with_node;
    stack_manipulation::go_through_stack(xyzY, true);
    onl();
    std::cout << "xyz after incryption: "
              << xyzY.current_node.data_associated_with_node;

    std::cout << "\nAmount of Lines in this program: " << __LINE__ + 2; std::cout << "\n";
    return 0;
}
